...
class ConsumerDataStoreDriver(object):
    ...
    def store(self):
        ...
        if not ret:
            ...
            try:
                with DataStore (self.db) as dbobj:
                    ...
                    insert_ret =dbobj.insert (query_str,insert_valuesi)
     ...
     def _get_insert_vaues(self):
            ...
            (header, payload) = self.get_header_payload ()
            try:
              _insert_values= [None,time.time(),header["correlation_id"],header["username"],header["ticket_num"],
                                                header["type"],json.dumps (payload),'pending']
            except Exception as e:
